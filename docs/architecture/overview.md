...
四层之间是**单向依赖**：编排层调用智能体层，智能体层消费质量层与状态层，状态层不知道上面任何一层的存在。这条约束是刻意的——它让 Story Bible 可以被 CLI、被任何外部工具（包括不属于本仓库的编辑器）独立读写。

---

## 3. 一章的生命周期

由 [`orchestrator.py`](../../meta_writing/orchestrator.py) 驱动：
...
智能体调用层（[`llm.py`](../../meta_writing/llm.py)）统一 3 次重试、指数退避（2/4/8 秒）。重试条件是子进程非零退出、stdout 非法 JSON、`is_error`、空结果与超时；三次耗尽抛 `AgentInvocationError`。超时默认 900 秒，可经环境变量调整。详见 [`agent-backend.md`](agent-backend.md)。
...
| 编排层：一章的完整流水线 | [`pipelines.md`](pipelines.md) |
| 智能体后端：探测、命令构造、成本 | [`agent-backend.md`](agent-backend.md) |
