...
面向**质量敏感的长期项目**的推荐循环。与全自动链路相比，它慢，但每一章的状态更新都经过人确认，不会出现「写到第 30 章才发现第 12 章的状态就错了」。

适用于 `workflow_mode: manual` 的项目。

---

## 1. 为什么手动

自动链路的三个 LLM 替代环节各有其失效模式：

| 环节 | 自动实现 | 失效模式 |
|------|---------|----------|
| 选分支 | `BranchSelector` | 倾向选「安全」分支，长期导致剧情平淡 |
| 验收 | 无 | `stalled` / `max_revisions` 的章节直接落盘 |
| 状态更新 | `BibleUpdater` | 抽取错误会累积，且没有人看得见 |

第三条最危险。Story Bible 的错误不会立刻显形——它在十几章之后以「角色行为不一致」的形式爆发，那时已经很难回溯是哪一章写坏的。

手动链路把这三处交回给人，代价是每章多花几分钟。
