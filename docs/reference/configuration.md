...
不要把密钥提交进仓库。放进 shell 环境或被 `.gitignore` 忽略的本地 `.env`。

### 凭据

| 变量 | 使用者 | 必需性 |
|------|--------|--------|
| `MINIMAX_API_KEY` | `LLMClient` | 手动链路**必需** |
| `ANTHROPIC_AUTH_TOKEN` | `LLMClient` 的兼容别名 | 可替代上一项 |
| `DEEPSEEK_API_KEY` | `DeepSeekClient` | 用 DeepSeek 写手时必需 |
| `ANTHROPIC_API_KEY` | `ClaudeClient` | 自动链路的编辑角色；缺失则降级 |
...
| `ANTHROPIC_BASE_URL` | `MINIMAX_BASE_URL` 的兼容别名 |
...
`LLMClient` 的凭据：`api_key` 构造参数 → `MINIMAX_API_KEY` → `ANTHROPIC_AUTH_TOKEN` → 空串。

`LLMClient` 的 base URL：`base_url` 参数 → `MINIMAX_BASE_URL` → `ANTHROPIC_BASE_URL` → 硬编码默认。

`auto_runner.py` 额外多一步：环境变量都缺失时，会读仓库根 `.env` 里以 `MINIMAX_API_KEY=` 或 `ANTHROPIC_AUTH_TOKEN=` 开头的行。仍缺失则 `sys.exit(1)`。

> **注意**：`ANTHROPIC_AUTH_TOKEN` 与 `ANTHROPIC_BASE_URL` 指向的是 **MiniMax**，不是 Anthropic。真正的 Anthropic 凭据是 `ANTHROPIC_API_KEY`。这组命名容易混淆，配置前请确认。
...
```json
{ "name": "rescue-male-lead", "workflow_mode": "manual" }
```

`workflow_mode` 取 `manual` | `automatic`，决定哪条链路可用。文件缺失 → 模式互斥失效。
...
累积的风格与连续性规则。手动链路由人维护；自动链路由 `LessonAccumulator` 追加。
...
自动链路有独立的同名常量 `MAX_REVISIONS = 5`（在 `auto_runner.py` 内）。**两处需同步修改。**

### 上下文预算 — [`compressor.py`](../../meta_writing/story_bible/compressor.py)

| 参数 | 值 | 说明 |
|------|----|------|
| `StoryBibleCompressor(token_budget=…)` | `15000` | 构造参数，两条链路都用默认值 |
...
### LLM — [`llm.py`](../../meta_writing/llm.py)
...
| 超时 | 600 秒，连接 30 秒 |
| DeepSeek `max_tokens` 上限 | 8192（硬截断） |

各 agent 的 `max_tokens` 与 `temperature`：
...
| `third_editor_enabled` | 自动链路据此决定是否启用第三编辑 |

> `third_editor_enabled` 目前三个档案都是 `True`，且**手动链路不读取它**——`orchestrator.py` 无条件调用 ThemeAgent。只有 `auto_runner.py` 会检查。
...
| 陷阱 | 后果 | 规避 |
|------|------|------|
| 写手供应商名拼错 | `normalize_writer_provider` 静默 fallback 到 `deepseek`，不报错 | 只用 `minimax` / `deepseek` |
| 选 `deepseek` 写手但未设 `DEEPSEEK_API_KEY`（自动链路） | client 降级为 MiniMax，但模型名仍是 `deepseek-chat` → API 报错 | 成对配置 |
| 未设 `ANTHROPIC_API_KEY` 跑自动链路 | 静默降级，审稿严格度改变，输出格式不变 | 检查启动日志的 WARNING |
| 言情项目期待伏笔寿命 15 | `init` 的查表键 `言情` 不是合法 `Genre` 值，实得 20 | 手动填 `foreshadowing_max_age_chapters` |
| `.meta-writing-project.json` 缺失 | 工作流模式互斥失效，自动链路可覆写手动项目 | 保留该文件 |
